  ...
  teamTaskMutex.runExclusive(() => {
    try {
      ...
      if (idx >= 0) {
        ...
        // WS push: notify user of progress change
        if (wsBroadcastToUser) {
          const userId = l[idx].userId;
          if (userId) {
      ...
      try {
        ...
        (async () => {
          try {
            ...
            // WS push: notify user video is ready
            if (wsBroadcastToUser && task.userId && url) {
    ...
    async (req, res) => {
      ...
      try {
        ...
        async function setProgress(employee, statusStr) {
          ...
          // WS push
          if (wsBroadcastToUser && req.user && req.user.id) {
            wsBroadcastToUser(req.user.id, {
              type: 'team_progress',
              taskId: task._id,
              employee,
              status: statusStr,
