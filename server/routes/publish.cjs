...
module.exports = function (app) {
  // ─── AiToEarn MCP Proxy Endpoints ────────────────

  // GET /api/aiops/platforms — List available platforms
  ...
  app.get('/api/aiops/account-ui', authMiddleware, (req, res) => {
    const baseUrl =
      loadDB('settings').aitoearn_base_url ||
      process.env.AITO_EARN_BASE_URL ||
      'http://localhost:8090';
  ...
  app.post('/api/publish', authMiddleware, async (req, res) => {
    try {
      ...
      if (!targetAccounts.length) {
        // Fallback: try AiToEarn MCP if key configured
        if (CONFIG.aitoearnKey && content) {
          const platformMap = {
            twitter: 'twitter',
            youtube: 'youtube',
            tiktok: 'tiktok',
          ...
          const mcpArgs = {
            platform: (platforms && platforms.length
              ? platformMap[platforms[0]] || platforms[0]
              : 'twitter'),
            content: {
              ...
              texts: content.text
                ...
                : [],
              medias: content.urls
                ? content.urls.map((u) => ({ url: u }))
                : [],
            },
          };
          const mcpResp = await mcpCall(
            MCP_TOOLS.createPublishFlow,
            mcpArgs
          );
