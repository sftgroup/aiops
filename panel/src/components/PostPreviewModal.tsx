...
export default function PostPreviewModal({ text, imageUrl, prompt, onClose, onSaved }: Props) {
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      toast.error((e instanceof Error ? e.message : String(e)) || '发布失败');
    } finally {
      setPublishing(false);
    }
  };
      ...
      >
        ...
        >
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-dark-border shrink-0">
            ...
            <button onClick={onClose} className="p-2 text-gray-500 hover:text-white rounded-lg hover:bg-dark-hover transition-colors" style={{ minWidth: 44, minHeight: 44, display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }} aria-label="关闭">
              <X size={18} />
            </button>
          </div>
      ...
      >
        ...
        >
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-dark-border shrink-0">
            <h3 className="font-semibold text-white text-sm">📝 内容预览</h3>
            <div className="flex items-center gap-2">
              <div className="flex bg-dark-bg rounded-xl p-0.5 border border-dark-border">
                <button
                  onClick={() => setMode('preview')}
