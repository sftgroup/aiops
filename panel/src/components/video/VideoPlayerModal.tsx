...
export default function VideoPlayerModal({ src, title, onClose }: VideoPlayerModalProps) {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={title || t('playerModal.ariaLabel')}
    >
      <div
        ref={innerRef}
      ...
      >
        ...
        <div className="absolute bottom-0 left-0 right-0 z-10 flex items-center justify-center gap-3 px-4 py-2 bg-gradient-to-t from-black/60 to-transparent pointer-events-none">
          <span className="text-[10px] text-white/50 pointer-events-auto flex items-center gap-1">
            <span className="inline-flex items-center gap-0.5 bg-white/10 px-1.5 py-0.5 rounded text-[9px]">Space</span> {t('playerModal.spaceHint')}
          </span>
          <span className="text-[10px] text-white/50 pointer-events-auto flex items-center gap-1">
            <span className="inline-flex items-center gap-0.5 bg-white/10 px-1.5 py-0.5 rounded text-[9px]">← →</span> {t('playerModal.seekHint')}
          </span>
          <span className="text-[10px] text-white/50 pointer-events-auto flex items-center gap-1">
            <span className="inline-flex items-center gap-0.5 bg-white/10 px-1.5 py-0.5 rounded text-[9px]">↑ ↓</span> {t('playerModal.volumeHint')}
          </span>
          <span className="text-[10px] text-white/50 pointer-events-auto flex items-center gap-1">
            <span className="inline-flex items-center gap-0.5 bg-white/10 px-1.5 py-0.5 rounded text-[9px]">M</span> {t('playerModal.muteHint')}
          </span>
          <span className="text-[10px] text-white/50 pointer-events-auto flex items-center gap-1">
            <span className="inline-flex items-center gap-0.5 bg-white/10 px-1.5 py-0.5 rounded text-[9px]">F</span> {t('playerModal.fullscreenHint')}
          </span>
        </div>
      </div>
    </div>
